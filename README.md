...
O progresso está detalhado em [docs/PROXIMOS_PASSOS.md](docs/PROXIMOS_PASSOS.md). Em resumo: **MVP funcional** — Fases 0 a 3, 5 e 6 concluídas, e a Fase 4 (identificação do cliente por WhatsApp) entregue com provedor fake. O ciclo fecha ponta a ponta: o cliente monta o carrinho, se identifica, paga via Stripe, o restaurante recebe o pedido no kanban e o move até finalizar, e o cliente acompanha cada etapa ao vivo — com um WhatsApp a cada mudança de status.
...
2. **Checkout e pagamento** — em "Finalizar pedido", confirme o telefone (em modo de desenvolvimento o código aparece na tela), escolha entrega ou retirada e veja a taxa entrar e sair do total. Cadastre um endereço pelo CEP (ex.: `90010-150`) e finalize: o pedido é criado pela Edge Function `create-order`, que relê os preços no banco — mexer no valor pelo navegador não muda o que será cobrado. Em seguida vem o Checkout do Stripe, em modo teste: `4242 4242 4242 4242` aprova o pagamento, `4000 0000 0000 0002` recusa. Quem marca o pedido como pago é o webhook, nunca o front.
...
4. **Kanban de pedidos** — em **/admin/pedidos**, o pedido pago aparece sozinho (com som e destaque) e se move entre as cinco colunas, pela alça do card ou pelos botões do detalhe. O timer de cada card fica verde, laranja ou vermelho conforme a previsão; para ver as cores sem esperar, mude `estimated_ready_at` do pedido no Table Editor. Abra duas abas para ver a mudança aparecer na outra em segundos.
5. **Acompanhamento pelo cliente** — em **/`slug`/pedido/`id`** (o link também vem na tela de retorno do pagamento), a linha do tempo avança sozinha conforme o restaurante move o card, sem recarregar.
6. **Dados direto no Supabase** — no dashboard do projeto, em _Table Editor_ ou _SQL Editor_, é possível inspecionar as tabelas e o seed. Para popular/repopular o restaurante fictício, rode [supabase/seed.sql](supabase/seed.sql) no SQL Editor (é idempotente).
