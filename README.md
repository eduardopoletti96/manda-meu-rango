...
O progresso está detalhado em [docs/PROXIMOS_PASSOS.md](docs/PROXIMOS_PASSOS.md). Em resumo: **Fases 0 a 3 e 5 concluídas** e **Fase 4** (identificação do cliente por WhatsApp) entregue com provedor fake — projeto configurado, banco completo (schema, RLS multi-tenant, storage, seed), **painel do restaurante funcional**, **vitrine com carrinho** e **checkout completo com endereço, pedido calculado no servidor e pagamento via Stripe**. O caminho do cliente está de pé ponta a ponta; falta a operação do restaurante (Fase 6) para fechar o MVP.
